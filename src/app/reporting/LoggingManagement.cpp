...
LoggingManagement::LoggingManagement(void) :
    mpEventBuffer(nullptr), mpExchangeMgr(nullptr), mState(LoggingManagementStates::Idle), mBytesWritten(0)
{}
...
PriorityLevel LoggingManagement::GetCurrentPriority(chip::ClusterId aClusterId)
{
    ...
    return config.GetGlobalPriorityLevel();
}
    ...
    if (aReader.GetTag() == chip::TLV::ContextTag(chip::app::EventDataElement::kCsTag_DeltaSystemTimestamp))
    {
        if (ctx->mContext->mFirst) // First event gets a timestamp, subsequent ones get a delta T
        {
            err = ctx->mWriter->Put(chip::TLV::ContextTag(chip::app::EventDataElement::kCsTag_SystemTimestamp),
                                    ctx->mContext->mCurrentSystemTime.mValue);
        }
        else
        {
                                       ...
                                       chip::EventNumber & aEventNumber)
{
    ...
    VerifyOrExit(mState != LoggingManagementStates::Shutdown, err = CHIP_ERROR_INCORRECT_STATE);
                                              ...
                                              chip::EventNumber & aEventNumber)
{
    ...
    if ((apOptions != nullptr) && (apOptions->mTimestamp.mType == Timestamp::Type::kSystem))
    {
...
exit:
    ...
    else if (opts.mpEventSchema->mPriority <= GetCurrentPriority(opts.mpEventSchema->mClusterId))
    {
...
CHIP_ERROR LoggingManagement::SetLoggingEndpoint(chip::EventNumber * apEventEndpoints, size_t aNumPriorityLevels)
{
    ...
    while (eventBuffer != nullptr && aNumPriorityLevels > 0)
    {
...
void CircularEventBuffer::AddEvent(chip::app::reporting::Timestamp aEventTimestamp)
{
